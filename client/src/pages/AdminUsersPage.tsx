...
export const AdminUsersPage = ({ embedded = false }: { embedded?: boolean } = {}) => {
  ...
  const submitCreate = async () => {
    ...
    try {
      ...
      toast.success(t("pages.users.createdToast", { defaultValue: "Usuário criado" }));
      setCreating(false);
      await reload();
    } catch (e) {
      toastError(e);
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      toast.success(t("pages.users.updatedToast", { defaultValue: "Usuário atualizado" }));
      setEditing(null);
      await reload();
    } catch (e) {
      toast.error((e as Error).message);
  ...
  const requestRoleChange = (u: AuthUser, nextRole: "admin" | "user") => {
    ...
    if (!wantsAdmin && u.id === me?.id) {
      toast.error(t("pages.users.errRemoveOwnAdmin", { defaultValue: "Você não pode remover seu próprio papel de administrador." }));
      return;
    }
    if (!wantsAdmin && isCurrentlyAdmin && adminCount <= 1) {
      toast.error(t("pages.users.errNeedOneAdmin", { defaultValue: "É necessário pelo menos um administrador ativo." }));
      return;
    }
  ...
  return (
    <Wrapper>
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => !o && closeDialog()}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {editing
                ? t("pages.users.editHint", { defaultValue: "Atualize dados de acesso e filas vinculadas. Deixe a senha em branco para mantê-la." })
                : t("pages.users.newHint", { defaultValue: "Crie um novo usuário e selecione as filas que ele atenderá." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <Label htmlFor="u-email">{t("auth.email", { defaultValue: "Email" })}</Label>
                <Input
                  id="u-email"
                  type="email"
                  value={form.email}
              ...
              <div className="col-span-2">
                <Label htmlFor="u-name">{t("pages.users.nameLabel", { defaultValue: "Nome do usuário" })}</Label>
                <Input
                  id="u-name"
                  placeholder={t("pages.users.namePlaceholder", { defaultValue: "Ex.: João Silva — usado na assinatura do chat" })}
                  value={form.name}
              ...
              <div className="col-span-2">
                <Label htmlFor="u-pwd">
                  {t("auth.password", { defaultValue: "Senha" })} {editing && <span className="text-muted-foreground">({t("pages.users.optional", { defaultValue: "opcional" })})</span>}
                </Label>
                <Input
                  id="u-pwd"
                  type="password"
                  placeholder={editing ? t("pages.users.passwordKeepPlaceholder", { defaultValue: "Deixe em branco para manter" }) : t("pages.users.passwordMinPlaceholder", { defaultValue: "Mínimo 8 caracteres" })}
                  value={form.password}
                  onChange={(e) => setForm({ ...form, password: e.target.value })}
                />
              </div>
              <div>
                <Label htmlFor="u-company">{t("pages.users.companyLabel", { defaultValue: "Empresa" })}</Label>
                <Input
                  id="u-company"
            ...
            <div>
              <Label>{t("pages.users.linkedQueuesLabel", { defaultValue: "Filas vinculadas" })}</Label>
              {queues.length === 0 ? (
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("pages.users.noQueuesHint", { defaultValue: "Nenhuma fila cadastrada. Crie filas em" })} <span className="font-medium">{t("pages.queues.title", { defaultValue: "Filas" })}</span>.
                </p>
              ) : (
            ...
            <div>
              <Label>{t("pages.users.linkedConnectionsLabel", { defaultValue: "Conexões vinculadas" })}</Label>
              {sessions.length === 0 ? (
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("pages.users.noConnectionsHint", { defaultValue: "Nenhuma conexão cadastrada. Crie em" })} <span className="font-medium">{t("nav.connections", { defaultValue: "Conexões" })}</span>.
                </p>
              ) : (
                <div className="mt-1 flex flex-wrap gap-2">
                  {sessions.map((s) => {
            ...
            <div>
              <div className="flex items-center justify-between">
                <Label>{t("pages.users.accessProfileLabel", { defaultValue: "Perfil de acesso" })}</Label>
                {form.role !== "admin" && (
                  <div className="flex gap-2 text-[11px]">
                    ...
                    >
                      {t("pages.users.markAll", { defaultValue: "Marcar todos" })}
                    </button>
                    ...
                    >
                      {t("pages.users.clearAll", { defaultValue: "Limpar" })}
                    </button>
                  </div>
                )}
              </div>
              {form.role === "admin" ? (
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("pages.users.adminHint", { defaultValue: "Administradores têm acesso total — todas as permissões são concedidas automaticamente." })}
                </p>
              ) : (
          ...
          <DialogFooter>
            ...
            >
              ...
              {editing ? t("common.save", { defaultValue: "Salvar" }) : t("pages.kanban.create", { defaultValue: "Criar" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <ConfirmDialog
        ...
        description={toDelete ? t("pages.users.deleteDescription", { defaultValue: "{{email}} perderá acesso. Sessões e fluxos ficarão órfãos.", email: toDelete.email }) : undefined}
      ...
      <ConfirmDialog
        ...
        title={roleChange?.nextRole === "admin" ? t("pages.users.promoteTitle", { defaultValue: "Promover a administrador?" }) : t("pages.users.demoteTitle", { defaultValue: "Rebaixar para atendente?" })}
        description={
          roleChange
            ? roleChange.nextRole === "admin"
              ? t("pages.users.promoteDescription", { defaultValue: "{{email}} terá acesso total: gerenciar usuários, filas, empresas e conexões.", email: roleChange.user.email })
              : t("pages.users.demoteDescription", { defaultValue: "{{email}} perderá acesso às telas de administração e verá apenas as filas vinculadas.", email: roleChange.user.email })
            : undefined
        }
        confirmLabel={roleChange?.nextRole === "admin" ? t("pages.users.promote", { defaultValue: "Promover" }) : t("pages.users.demote", { defaultValue: "Rebaixar" })}
...
const RoleBadge = ({ role }: { role: "admin" | "user" }) => {
  ...
  return (
    ...
    >
      ...
      {isAdmin ? t("pages.users.roleAdmin", { defaultValue: "Admin" }) : t("pages.users.roleAgent", { defaultValue: "Atendente" })}
    </span>
  );
};
      ...
      {(["user", "admin"] as const).map((r) => {
        ...
        return (
          ...
          >
            ...
            {r === "admin" ? t("pages.users.roleAdmin", { defaultValue: "Admin" }) : t("pages.users.roleAgent", { defaultValue: "Atendente" })}
          </button>
        );
      })}
    </div>
