...
export const ensureCallsWired = (): void => {
  ...
  eventStream.on((ev: BrokerEvent) => {
    if (ev.type === "call-list") {
      useCalls.setState((s) => ({
        calls: ev.calls.map((next) => {
          const current = s.calls.find((call) => call.callId === next.callId);
          if (!current) return next;
          return {
            ...current,
            ...next,
            sessionId: next.sessionId || current.sessionId,
            peer: next.peer || current.peer,
      ...
      useCalls.setState((s) => ({
        // Preserva peer/startedAt quando o evento vem sem esses campos —
        // caso contrário a chamada "perde" o número ao ser atendida e o
        // botão de desligar deixa de casar com o chat aberto.
        calls: s.calls.map((c) =>
          c.callId === ev.id
            ? {
                ...c,
                sessionId: ev.sessionId || c.sessionId,
                status: ev.status,
                peer: ev.peer || c.peer,
                startedAt: ev.startedAt || c.startedAt,
              }
            : c,
        ),
      }));
