...
import { listFlows, parseGraph } from "@/services/flows";
...
export const EditConnectionModal = ({ open, onOpenChange, session, onSaved }: Props) => {
  ...
  useEffect(() => {
    ...
    void listFlows()
      .then((fs) => setFlows(fs.filter((f) => parseGraph(f.graph).kind === "chat")))
      .catch(() => {});
  }, [open, session]);
  ...
  const onSave = async () => {
    ...
    try {
      await updateSession(session.id, {
        ...
        // Precisa ir junto: o servidor troca o valor pelo que vier no corpo, e
        // omitir este campo apagava o fluxo vinculado a cada edição da conexão.
        chatFlowId,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md overflow-hidden p-0">
        ...
        <div className="max-h-[70vh] space-y-5 overflow-y-auto px-6 py-5">
          ...
          <div>
            <Label htmlFor="cflow">Fluxo do chatbot</Label>
            <select
              id="cflow"
              value={chatFlowId}
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">
              Mensagem recebida neste número entra neste fluxo. Sem fluxo, a conversa vai direto para a fila.
            </p>
          </div>
          ...
          <BusinessHoursEditor
            scope="session"
            scopeId={session.id}
            fallbackMessage={session.outOfHoursMessage ?? ""}
          />
        </div>
