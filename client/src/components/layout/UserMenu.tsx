  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <span className="grid h-7 w-7 shrink-0 place-items-center overflow-hidden rounded-full bg-nav-icon/10 text-[11px] font-semibold text-nav-icon">
            {avatarSrc ? (
      ...
      <DropdownMenuContent align="end" className="w-64">
        <DropdownMenuLabel className="flex items-start gap-3 py-3">
          <span className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full bg-nav-icon/10 text-sm font-semibold text-nav-icon">
            {avatarSrc ? (
              <img src={avatarSrc} alt="" className="h-full w-full object-cover" />
            ) : (
              initials
