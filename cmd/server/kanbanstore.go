...
type kanbanColumn struct {
	...
	// SLAHours is the per-stage SLA in hours. Cards older than this in the
	// column are flagged as out of SLA. 0 disables it.
...
func (s *kanbanStore) CreateBoard(ctx context.Context, name, color, description, ownerID string) (kanbanBoard, error) {
	...
	for i, n := range defaults {
		if _, err := s.CreateColumn(ctx, b.ID, n, colors[i], stypes[i], slas[i]); err != nil {
			return b, err
		}
	}
	return b, nil
...
func (s *kanbanStore) ListColumns(ctx context.Context, boardID string) ([]kanbanColumn, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id,board_id,name,color,position,COALESCE(stage_type,'open'),COALESCE(sla_hours,0),created_at FROM kanban_columns WHERE board_id = ? ORDER BY position ASC, created_at ASC`,
		boardID)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		var c kanbanColumn
		if err := rows.Scan(&c.ID, &c.BoardID, &c.Name, &c.Color, &c.Position, &c.StageType, &c.SLAHours, &c.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, c)
	}
...
func (s *kanbanStore) CreateColumn(ctx context.Context, boardID, name, color, stageType string, slaHours int) (kanbanColumn, error) {
	name = strings.TrimSpace(name)
	if name == "" {
	...
	c := kanbanColumn{ID: newID(), BoardID: boardID, Name: name, Color: color, Position: pos, StageType: stageType, SLAHours: slaHours, CreatedAt: time.Now().Unix()}
	if _, err := s.db.ExecContext(ctx,
		`INSERT INTO kanban_columns (id,board_id,name,color,position,stage_type,sla_hours,created_at) VALUES (?,?,?,?,?,?,?,?)`,
		c.ID, c.BoardID, c.Name, c.Color, c.Position, c.StageType, c.SLAHours, c.CreatedAt); err != nil {
		return kanbanColumn{}, err
	}
	return c, nil
}

func (s *kanbanStore) UpdateColumn(ctx context.Context, id, name, color, stageType string, slaHours int) error {
	if slaHours < 0 {
		slaHours = 0
	}
	res, err := s.db.ExecContext(ctx,
		`UPDATE kanban_columns SET name=?, color=?, stage_type=?, sla_hours=? WHERE id=?`,
		strings.TrimSpace(name), color, normalizeStageType(stageType), slaHours, id)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (s *kanbanStore) GetColumn(ctx context.Context, id string) (kanbanColumn, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id,board_id,name,color,position,COALESCE(stage_type,'open'),COALESCE(sla_hours,0),created_at FROM kanban_columns WHERE id=?`, id)
	var c kanbanColumn
	if err := row.Scan(&c.ID, &c.BoardID, &c.Name, &c.Color, &c.Position, &c.StageType, &c.SLAHours, &c.CreatedAt); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
