...
func persistCallRecording(ctx context.Context, store *callStore, sessionID, callID string, buf *callRecorderBuf) (RecordingInfo, error) {
	...
	// Call teardown persists the call row and flushes audio on separate paths.
	// The WAV can finish first, so retry briefly until the historical row exists
	// instead of silently losing the recording metadata.
	var setErr error
	for attempt := 0; attempt < 20; attempt++ {
		setErr = store.SetRecording(ctx, callID, info)
		if setErr == nil {
			return info, nil
		}
		select {
