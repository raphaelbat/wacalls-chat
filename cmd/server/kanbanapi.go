...
func (s *server) handleKBColumnCreate(w http.ResponseWriter, r *http.Request) {
	...
	c, err := s.kanban.CreateColumn(r.Context(), boardID, body.Name, body.Color, body.StageType, body.SLAHours)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
...
func (s *server) handleKBColumnUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.kanban.UpdateColumn(r.Context(), id, body.Name, body.Color, body.StageType, body.SLAHours); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	w.WriteHeader(http.StatusNoContent)
